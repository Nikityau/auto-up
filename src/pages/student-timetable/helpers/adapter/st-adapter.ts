import {ResSchedule} from "../../../timetable/helpers/adapter/interface/res-schedule";
...
export const stScheduleAdapter = async (data: ResSchedule[], token: string): Promise<Res> => {
    ...
    for (let sch of data) {
        if(!groupCache[sch.group]) {
            const resGroup = await axios.get(`${baseUrl}/api/v1/study_groups/`, {
                headers: {
